...
class Article extends Model
{
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['meta_description'])) {
                DB::table('site_managements')->insert(
                    [
                        'meta_key' => 'article_meta-desc-' . $article->id, 'meta_value' => $request['meta_description'],
     ...
     * @return relation
     */
    public function createArticleAPI(Request $request, $image_size)
    {
        if (!empty($request)) {
            $path = Helper::PublicPath() . '/uploads/users/'.$request['id'].'/articles/';
            $article = new Article();
            $article->title = filter_var($request['title'], FILTER_SANITIZE_STRING);
            $article->slug = filter_var($request['title'], FILTER_SANITIZE_STRING);
            $article->description = $request['description'];
            ...
            $article->author()->associate($user_id);
            if (!empty($request['img']['base64_string'])) {
                $image = base64_decode($request['img']['base64_string']);
                if (!empty($image_size)) {
                    foreach ($image_size as $key => $size) {
                        ...
                        $resize_img->save($path . $key . '-' . $request['img']['name']);
                    }
                }
                $article->image = filter_var($request['img']['name'], FILTER_SANITIZE_STRING);
            } else {
                $article->image = null;
            }
            $article->save();
            ...
            if (!empty($cats)) {
                foreach ($cats as $cat) {
                    $this->categories()->attach($cat, ['article_id' => $article->id]);
                }
            }
            return 'success';
        } else {
    ...
    public function updateArticle($request, $article_id, $user_id)
    {
        if (!empty($request)) {
            ...
            if ($article->author_id == $user_id) {
                ...
                if (!empty($request['meta_description'])) {
                    DB::table('site_managements')->select('meta_value')->where('meta_key', 'article_meta-desc-'.$article->id)->delete();
                    DB::table('site_managements')->insert(
                        [
                            'meta_key' => 'article_meta-desc-' . $article->id, 'meta_value' => $request['meta_description'],
                            "created_at" => Carbon::now(), "updated_at" => Carbon::now()
