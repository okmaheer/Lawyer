...
class ArticleController extends Controller
{
    ...
    {
        if (!empty($slug)) {
            $article = $this->article::where('slug', $slug)->where('author_id', Auth::user()->id)->first();
            if (!empty($article)) {
                $categories = array_pluck(Category::all()->toArray(), 'title', 'id');
                $article_meta_desc  = DB::table('site_managements')->select('meta_value')->where('meta_key', 'article_meta-desc-'.$article->id)->first();
                $meta_description = !empty($article_meta_desc) && !empty($article_meta_desc->meta_value) ? $article_meta_desc->meta_value : '';
                return View(
                    'back-end.articles.edit',
                    compact('article', 'categories', 'meta_description')
                );
            } else {
    ...
    public function articleListing($category = '')
    {
        ...
        $article_list_meta_title = !empty($inner_page) && !empty($inner_page['article_meta_title']) ? $inner_page['article_meta_title'] : trans('lang.search_results');
        $article_list_meta_desc = !empty($inner_page) && !empty($inner_page['article_meta_desc']) ? $inner_page['article_meta_desc'] : trans('lang.search_results');
        if (file_exists(resource_path('views/extend/front-end/articles/index.blade.php'))) {
            return View(
                'extend.front-end.articles.index',
                compact(
                    'article_list_meta_title',
                    'article_list_meta_desc',
                    'categories',
                    'featured_articles',
                    'saved_articles',
                    'download_app_img',
        ...
        if (!empty($slug)) {
            ...
            if (!empty($article)) {
                ...
                if (file_exists(resource_path('views/extend/front-end/articles/show.blade.php'))) {
                    return View(
                        'extend.front-end.articles.show',
                        compact(
                            'article_meta_desc',
                            'article',
                            'categories',
                ...
                } elseif (file_exists(resource_path('views/front-end/articles/show.blade.php'))) {
                    return View(
