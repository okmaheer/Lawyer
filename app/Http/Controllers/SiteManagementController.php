...
class SiteManagementController extends Controller
{
    ...
    public function workSettings()
    {
        if (Auth::user()) {
            ...
            $home_how_works_tabs = $this->settings::getMetaValue('how_work_tabs'); //How it Works Tabs
            $tabs_unserialize_array = SiteManagement::getMetaValue('services_tab_sec'); //Services Tabs
            if (file_exists(resource_path('views/extend/back-end/admin/settings/how-it-work/index.blade.php'))) {
                return view(
                    'extend.back-end.admin.settings.how-it-work.index',
                    compact(
                        'how_works_title',
                        'how_works_subtitle',
                        'how_works_desc',
                ...
                return view(
                    'back-end.admin.settings.how-it-work.index',
                    compact(
                        'how_works_title',
                        'how_works_subtitle',
                        'how_works_desc',
    ...
    public function serviceSettings()
    {
        if (Auth::user()) {
            $tabs_unserialize_array = SiteManagement::getMetaValue('services_tab_sec'); //Services Tabs
            if (file_exists(resource_path('views/extend/back-end/admin/settings/services-settings.blade.php'))) {
                return view(
                    'extend.back-end.admin.settings.services-settings',
                    compact(
                        'tabs_unserialize_array'
                    )
                );
                ...
                return view(
                    'back-end.admin.settings.services-settings',
                    compact(
                        'tabs_unserialize_array'
                    )
                );
    ...
    public function generalSettings()
    {
        if (Auth::user()) {
            ...
            $selected_registration_type = !empty($register_form) && !empty($register_form['registration_type']) ? $register_form['registration_type'] : '';
            $selected_verification_type = !empty($register_form) && !empty($register_form['verification_type']) ? $register_form['verification_type'] : 'admin_verify';
            ...
            $selected_header = !empty($general_settings) && !empty($general_settings['header_style']) ? $general_settings['header_style'] : 'headerv1' ;
            ...
            $android_icon = !empty($footer_settings) && !empty($footer_settings['android_icon']) ? $footer_settings['android_icon'] : '';
            $ios_icon = !empty($footer_settings) && !empty($footer_settings['ios_icon']) ? $footer_settings['ios_icon'] : '';
            $android_url = !empty($footer_settings) && !empty($footer_settings['android_url']) ? $footer_settings['android_url'] : '';
            $ios_url = !empty($footer_settings) && !empty($footer_settings['ios_url']) ? $footer_settings['ios_url'] : '';
            $app_sec_title = !empty($footer_settings) && !empty($footer_settings['app_sec_title']) ? $footer_settings['app_sec_title'] : '';
            ...
            $article_meta_title = !empty($inner_page) && !empty($inner_page['article_meta_title']) ? $inner_page['article_meta_title'] : '';
            $article_meta_desc = !empty($inner_page) && !empty($inner_page['article_meta_desc']) ? $inner_page['article_meta_desc'] : '';
            ...
            $search_list_selected_header = !empty($inner_page) && !empty($inner_page['search_list_header_style']) ? $inner_page['search_list_header_style'] : 'headerv1';
            $article_selected_header = !empty($inner_page) && !empty($inner_page['article_header_style']) ? $inner_page['article_header_style'] : 'headerv1';
            $forum_selected_header = !empty($inner_page) && !empty($inner_page['forum_header_style']) ? $inner_page['forum_header_style'] : 'headerv1';
            $search_logo = !empty($inner_page) && !empty($inner_page['search_logo']) ? $inner_page['search_logo'] : '';
            $article_logo = !empty($inner_page) && !empty($inner_page['article_logo']) ? $inner_page['article_logo'] : '';
            $forum_logo = !empty($inner_page) && !empty($inner_page['forum_logo']) ? $inner_page['forum_logo'] : '';
            ...
            $pages = Page::get()->pluck('title', 'id')->toArray();
            $get_selected_homepage = DB::table('site_managements')->select('meta_value')->where('meta_key', 'homepage')->get()->first();
            $selected_homepage =  !empty($get_selected_homepage->meta_value) ? $get_selected_homepage->meta_value : '';
            $registration_type = Helper::getRegisterationTypes();
            $verification_types = Helper::getVerificationTypes();
            $search_form_types = Helper::getSearchFormTypes();
            if (file_exists(resource_path('views/extend/back-end/admin/settings/general/index.blade.php'))) {
                return view(
                    ...
                    compact(
                        ...
                        'search_form_types',
                        'selected_verification_type',
                        'verification_types',
                        'selected_registration_type',
                        'registration_type',
                        'app_sec_title',
                        'ios_url',
                        'android_url',
                        ...
                        'article_logo',
                        'search_logo',
                        'forum_selected_header',
                        'article_selected_header',
                        'search_list_selected_header',
                        'selected_header',
                        'menu_pages_1',
                        'menu_title_1',
                        'selected_homepage',
     ...
     * @return \Illuminate\Http\Response
     */
    public function storeHomepageSettings(Request $request)
    {
        $server = Helper::doctieIsDemoSiteAjax();
        if (!empty($server)) {
            $response['type'] = 'error';
        ...
        if (!empty($request)) {
            $homepage_settings = $this->settings->saveHomepageSettings($request);
            if ($homepage_settings == "success") {
                $json['type'] = 'success';
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.settings_saved');
                return $json;
            } elseif ($homepage_settings == "error") {
                $json['type'] = 'error';
                return $json;
            }
        } else {
    ...
    public function getBookingSwicthSettings()
    {
        ...
        if (!empty($settings['enable_call_appointment'])) {
            if ($settings['enable_call_appointment'] == 'true') {
                $json['enable_call_appointment'] = 'true';
            }
        }
        if (!empty($settings['enable_booking_private_clinic'])) {
            if ($settings['enable_booking_private_clinic'] == 'true') {
                $json['enable_booking_private_clinic'] = 'true';
            }
        }
        return $json;
    }
    ...
    public function importUpdate()
    {
        ...
        // if (Schema::hasTable('metas') && config('app.version') == 'v1.5') {
        //     $count = DB::table('metas')->count();
        //     if ($count == 0) {
        //         \Artisan::call('db:seed', ['--class' => 'PageSeeder', '--force' => true]);
        //         \Artisan::call('db:seed', ['--class' => 'SliderSeeder', '--force' => true]);
        //         \Artisan::call('db:seed', ['--class' => 'MetaSeeder', '--force' => true]);
        //     }
        // }
        $json['type'] = 'success';
        return $json;
    ...
    public function getPageOption(Request $request)
    {
        ...
        if (!empty($request['page_id'])) {
            ...
            if (!empty($page)) {
                ...
                $show_page = !empty($meta) && !empty($meta['show_page']) ? $meta['show_page'] : '';
                $json['type'] = 'success';
    ...
    public function getChatDisplaySetting()
    {
        ...
        if (!empty($settings['enable_loader'])) {
            if ($settings['enable_loader'] == 'true') {
                $json['enable_loader'] = 'true';
            } else if ($settings['enable_loader'] == 'false') {
    ...
    public function getInnerPageSettings()
    {
        ...
        if (!empty($inner_page_settings['show_forum_menu'])) {
            if ($inner_page_settings['show_forum_menu'] == 'true') {
                $json['show_forum_menu'] = 'true';
            } else {
                $json['show_forum_menu'] = 'false';
            }
        } else {
            $json['show_forum_menu'] = 'true';
        }
        if (!empty($inner_page_settings['show_article_menu'])) {
            if ($inner_page_settings['show_article_menu'] == 'true') {
                $json['show_article_menu'] = 'true';
            } else {
                $json['show_article_menu'] = 'false';
            }
        } else {
            $json['show_article_menu'] = 'true';
        }
        if (!empty($inner_page_settings['show_search_menu'])) {
            if ($inner_page_settings['show_search_menu'] == 'true') {
                $json['show_search_menu'] = 'true';
            } else {
                $json['show_search_menu'] = 'false';
            }
        } else {
            $json['show_search_menu'] = 'true';
        }
        if (!empty($inner_page_settings['show_search_list_header_styling'])) {
            if ($inner_page_settings['show_search_list_header_styling'] == 'true') {
                $json['show_search_list_header_styling'] = 'true';
            }
        }
        if (!empty($inner_page_settings['show_article_header_styling'])) {
            if ($inner_page_settings['show_article_header_styling'] == 'true') {
                $json['show_article_header_styling'] = 'true';
            }
        }
        if (!empty($inner_page_settings['show_forum_header_styling'])) {
            if ($inner_page_settings['show_forum_header_styling'] == 'true') {
                $json['show_forum_header_styling'] = 'true';
            }
        }
        $json['search_menu_color'] = !empty($inner_page_settings['search_menu_color']) ? $inner_page_settings['search_menu_color'] : '';
        $json['search_hover_color'] = !empty($inner_page_settings['search_hover_color']) ? $inner_page_settings['search_hover_color'] : '';
        $json['search_menu_text_color'] = !empty($inner_page_settings['search_menu_text_color']) ? $inner_page_settings['search_menu_text_color'] : '';
        $json['article_menu_color'] = !empty($inner_page_settings['article_menu_color']) ? $inner_page_settings['article_menu_color'] : '';
        $json['article_hover_color'] = !empty($inner_page_settings['article_hover_color']) ? $inner_page_settings['article_hover_color'] : '';
        $json['article_menu_text_color'] = !empty($inner_page_settings['article_menu_text_color']) ? $inner_page_settings['article_menu_text_color'] : '';
        $json['forum_menu_color'] = !empty($inner_page_settings['forum_menu_color']) ? $inner_page_settings['forum_menu_color'] : '';
        $json['forum_hover_color'] = !empty($inner_page_settings['forum_hover_color']) ? $inner_page_settings['forum_hover_color'] : '';
        $json['forum_menu_text_color'] = !empty($inner_page_settings['forum_menu_text_color']) ? $inner_page_settings['forum_menu_text_color'] : '';
        return $json;
    }
     ...
     * @return View
     */
    public function storeMenuSettings(Request $request)
    {
        $server = Helper::doctieIsDemoSiteAjax();
        if (!empty($server)) {
            $response['type'] = 'error';
        ...
        if (!empty($request['menu'])) {
            $store_menu_settings
                = $this->settings->saveMenuSettings($request['menu']);
            if ($store_menu_settings == "success") {
                $json['type'] = 'success';
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.settings_saved');
                return $json;
            } elseif ($store_menu_settings == "lang_not_found") {
                $json['type'] = 'error';
                $json['message'] = trans('lang.lang_not_found');
                return $json;
            } else {
     ...
     * @return View
     */
    public function getMenuColorSetting()
    {
        $json = array();
        $settings = !empty(SiteManagement::getMetaValue('menu_settings')) ? SiteManagement::getMetaValue('menu_settings') : array();
        if (!empty($settings['menu_color'])) {
            $json['menu_color'] = $settings['menu_color'];
        }
        if (!empty($settings['menu_hover_color'])) {
            $json['menu_hover_color'] = $settings['menu_hover_color'];
        }
    ...
    public function getPagesOrder()
    {
        ...
        if (!empty($saved_pages_array)) {
            $json['storedPages'] = $saved_pages_array;
        }
        return $json;
    }

    /**
     * Get Inner Pages Order settings
     *
     * @access public
     *
     * @return View
     */
    public function getInnerPagesOrder()
    {
        $json = array();
        $settings = !empty(SiteManagement::getMetaValue('menu_settings')) ? SiteManagement::getMetaValue('menu_settings') : array();
        $saved_pages_array = array();
        if (!empty($settings['pages'])) {
            ...
            foreach ($settings['pages'] as $key => $value) {
                $saved_pages_array[$count]['id'] =  $value['id'];
                $saved_pages_array[$count]['title'] = $value['title']; 
                $saved_pages_array[$count]['type'] =  $value['type'];
                $saved_pages_array[$count]['order'] =  $value['order'];
                $count++;
            }
        }
        if (!empty($saved_pages_array)) {
     ...
     * @return View
     */
    public function getRegistrationTypeSettings()
    {
        
        $json = array();
        $reg_form_setting = !empty(SiteManagement::getMetaValue('reg_form_settings')) ? SiteManagement::getMetaValue('reg_form_settings') : array();
        if (!empty($reg_form_setting['registration_type'])) {
            $json['registration_type'] = $reg_form_setting['registration_type'];
            $json['enable_verification'] = $reg_form_setting['enable_verification'];
