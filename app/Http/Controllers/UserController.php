...
class UserController extends Controller
{
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($template->id)) {
                            ...
                            try {
                                ...
                                Session::flash('error', trans('lang.email_warning'));
                                return Redirect::back();
                            }
                        }
                    }
    ...
    public function userProfileSettings($role_type)
    {
        if (Auth::user()) {
            ...
            $location_title = !empty($user_meta->location_title) ? $user_meta->location_title : '';
            $location_image = !empty($user_meta->location_image) ? $user_meta->location_image : '';
            ...
            $location_title = !empty($default_location) && !empty($default_location['location_title']) ? $default_location['location_title'] : '';
            $personal_mob_num = !empty(Auth::user()->metaValue('personal_mob_num')) ? Auth::user()->metaValue('personal_mob_num')['meta_value'] : '';
     ...
     * @return \Illuminate\Http\Response
     */
    public function storeDefaultLocation(Request $request)
    {
        $server = Helper::doctieIsDemoSiteAjax();
        if (!empty($server)) {
            $response['type'] = 'error';
        ...
        if (Auth::user()) {
            $profile = $this->user_meta->storeDefaultLocation($request, Auth::user()->id);
            if ($profile == 'success') {
                $json['type'] = 'success';
                $json['role'] = Helper::getAuthRoleType();
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.location_details_saved_successfully');
                return $json;
            }
        } else {
            $json['type'] = 'error';
    ...
    public function getPayouts(Request $request)
    {
        ...
        $payout_status = Helper::getPayoutStatus();
        if (file_exists(resource_path('views/extend/back-end/admin/payouts.blade.php'))) {
            return view(
                'extend.back-end.admin.payouts',
                compact('payouts', 'years', 'selected_year', 'months', 'selected_month', 'payout_status')
            );
        } else {
            return view(
        ...
        if (!empty($request['id'])) {
            ...
            if (!empty($request['projects_ids'])) {
                ...
                foreach ($projects_ids as $key => $id) {
                    ...
                    $appointment->payout_progress = 'processed';
                    $appointment->save();
                }
            }
            $user = User::find($payout->user_id);
            $email_params = array();
            $email_params['doctor_name'] = Helper::getUserName($payout->user_id);
            $email_params['total_amount'] = $payout->amount;
            $template_data = Helper::getDoctorPayoutProcessedEmailContent();
            Mail::to($user->email)
                ->send(
                    new DoctorEmailMailable(
                        'doctor_email_payout_processed',
                        $template_data,
                        $email_params
                    )
                );
    ...
    public function updateUserProfileSettings(Request $request)
    {
        ...
        if (!empty($request['id'])) {
            $role_type = Helper::getRoleTypeByUserID($request['id']);
