...
class SiteManagement extends Model
{
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['slider_bg_img'])) {
                ...
                if (!empty($slider_bg_img_data->meta_value)) {
                    DB::table('site_managements')->where('meta_key', '=', 'slider_bg_img')->delete();
                }
                DB::table('site_managements')->insert(
                    [
    ...
    public static function saveHomeAboutUsSettings($request)
    {
        if (!empty($request)) {
            ...
            if (!empty($existing_data)) {
                DB::table('site_managements')->where('meta_key', '=', 'home_about_us_sec')->delete();
            }
            DB::table('site_managements')->insert(
                [
    ...
    public static function saveHowItWorksSettings($request)
    {
        if (!empty($request)) {
            ...
            if (!empty($existing_data)) {
                DB::table('site_managements')->where('meta_key', '=', 'home_how_works_sec')->delete();
            }
            DB::table('site_managements')->insert(
                [
    ...
    public static function saveServiceTabsSettings($request)
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['show_services_section'])) {
                DB::table('site_managements')->insert(
                    [
    ...
    public static function saveFooterSettings($request, $image_size = array())
    {
        if (!empty($request)) {
            ...
            if (!empty($request['android_icon'])) {
                if (file_exists($old_path . '/' . $request['android_icon'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['android_icon'];
                    rename($old_path . '/' . $request['android_icon'], $new_path . '/' . $filename);
                    $request['android_icon'] = $filename;
                }
            }
            if (!empty($request['ios_icon'])) {
                if (file_exists($old_path . '/' . $request['ios_icon'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $request['ios_icon'];
                    rename($old_path . '/' . $request['ios_icon'], $new_path . '/' . $filename);
                    $request['ios_icon'] = $filename;
                }
            }
    ...
    public static function saveArticleSectionSettings($request)
    {
        if (!empty($request)) {
            ...
            if (!empty($existing_data)) {
                DB::table('site_managements')->where('meta_key', '=', 'article_section')->delete();
            }
            DB::table('site_managements')->insert(
                [
    ...
    public static function saveInnerPageSettings($request)
    {
        ...
        if (!empty($request)) {
            ...
            $inner_page_data_array['article_meta_title'] = $inner_page['article_meta_title'];
            $inner_page_data_array['article_meta_desc'] = $inner_page['article_meta_desc'];
            $inner_page_data_array['health_meta_title'] = $inner_page['health_meta_title'];
            $inner_page_data_array['health_meta_desc'] = $inner_page['health_meta_desc'];
            ...
            $inner_page_data_array['show_forum_menu'] = $inner_page['show_forum_menu'];
            $inner_page_data_array['show_article_menu'] = $inner_page['show_article_menu'];
            $inner_page_data_array['show_search_menu'] = $inner_page['show_search_menu'];
            $inner_page_data_array['search_list_header_style'] = $inner_page['search_list_header_style'];
            $inner_page_data_array['article_header_style'] = $inner_page['article_header_style'];
            $inner_page_data_array['forum_header_style'] = $inner_page['forum_header_style'];
            $inner_page_data_array['show_search_list_header_styling'] = $inner_page['show_search_list_header_styling'];
            $inner_page_data_array['search_menu_color'] = !empty($inner_page['search_menu_color']) ? $inner_page['search_menu_color'] : '';
            $inner_page_data_array['search_hover_color'] = !empty($inner_page['search_hover_color']) ? $inner_page['search_hover_color'] : '';
            $inner_page_data_array['search_menu_text_color'] = !empty($inner_page['search_menu_text_color']) ? $inner_page['search_menu_text_color'] : '';
            $inner_page_data_array['show_article_header_styling'] = $inner_page['show_article_header_styling'];
            $inner_page_data_array['article_menu_color'] = !empty($inner_page['article_menu_color']) ?  $inner_page['article_menu_color'] : '';
            $inner_page_data_array['article_hover_color'] = !empty($inner_page['article_hover_color']) ?  $inner_page['article_hover_color'] : '';
            $inner_page_data_array['article_menu_text_color'] = !empty($inner_page['article_menu_text_color']) ? $inner_page['article_menu_text_color'] : '';
            $inner_page_data_array['show_forum_header_styling'] = $inner_page['show_forum_header_styling'];
            $inner_page_data_array['forum_menu_color'] = !empty($inner_page['forum_menu_color']) ? $inner_page['forum_menu_color'] : '';
            $inner_page_data_array['forum_hover_color'] = !empty($inner_page['forum_hover_color']) ? $inner_page['forum_hover_color'] : '';
            $inner_page_data_array['forum_menu_text_color'] = !empty($inner_page['forum_menu_text_color']) ? $inner_page['forum_menu_text_color'] : '';
            $old_path = Helper::PublicPath() . '/uploads/settings/temp';
            $new_path = Helper::PublicPath() . '/uploads/settings/inner-page';
            if (!empty($inner_page['search_logo'])) {
                if (file_exists($old_path . '/' . $inner_page['search_logo'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $inner_page['search_logo'];
                    if (!empty($image_size)) {
                        foreach ($image_size as $size) {
                            rename($old_path . '/' . $size . '-' . $inner_page['search_logo'], $new_path . '/' . $size . '-' . $filename);
                        }
                        rename($old_path . '/' . $inner_page['search_logo'], $new_path . '/' . $filename);
                    } else {
                        rename($old_path . '/' . $inner_page['search_logo'], $new_path . '/' . $filename);
                    }
                    $inner_page['search_logo'] = $filename;
                }
            }
            if (!empty($inner_page['article_logo'])) {
                if (file_exists($old_path . '/' . $inner_page['article_logo'])) {
                    ...
                    if (!empty($image_size)) {
                        foreach ($image_size as $size) {
                            rename($old_path . '/' . $size . '-' . $inner_page['article_logo'], $new_path . '/' . $size . '-' . $filename);
                        }
                        rename($old_path . '/' . $inner_page['article_logo'], $new_path . '/' . $filename);
                    } else {
                        rename($old_path . '/' . $inner_page['article_logo'], $new_path . '/' . $filename);
                    }
                    $inner_page['article_logo'] = $filename;
                }
            }
            if (!empty($inner_page['forum_logo'])) {
                if (file_exists($old_path . '/' . $inner_page['forum_logo'])) {
                    if (!file_exists($new_path)) {
                        File::makeDirectory($new_path, 0755, true, true);
                    }
                    $filename = time() . '-' . $inner_page['forum_logo'];
                    if (!empty($image_size)) {
                        foreach ($image_size as $size) {
                            rename($old_path . '/' . $size . '-' . $inner_page['forum_logo'], $new_path . '/' . $size . '-' . $filename);
                        }
                        rename($old_path . '/' . $inner_page['forum_logo'], $new_path . '/' . $filename);
                    } else {
                        rename($old_path . '/' . $inner_page['forum_logo'], $new_path . '/' . $filename);
                    }
                    $inner_page['forum_logo'] = $filename;
                }
            }
            $inner_page_data_array['search_logo'] = !empty($inner_page['search_logo']) ? $inner_page['search_logo'] : '';
            $inner_page_data_array['article_logo'] = !empty($inner_page['article_logo']) ? $inner_page['article_logo'] : '';
            $inner_page_data_array['forum_logo'] = !empty($inner_page['forum_logo']) ? $inner_page['forum_logo'] : '';
     ...
     * @return \Illuminate\Http\Response
     */
    public static function saveHomepageSettings($request)
    {
        if (!empty($request)) {
            $homepage = $request['homepage'];
            $existing_data = DB::table('site_managements')->select('meta_value')->where('meta_key', 'homepage')->get()->first();
            if (!empty($existing_data)) {
                DB::table('site_managements')->where('meta_key', '=', 'homepage')->delete();
            }
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'homepage', 'meta_value' => $homepage,
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            return 'success';
    ...
    public static function saveMenuSettings($menu_settings)
    {
        ...
        if (!empty($menu_settings)) {
            ...
            if (!empty($menu_settings['inner_pages'])) {
                foreach ($menu_settings['inner_pages'] as $key => $value) {
