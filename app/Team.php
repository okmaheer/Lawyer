...
class Team extends Model
{
    ...
    public function saveAppointmentLocation($request, $user_id = '')
    {
        ...
        if (!empty($request['slots'])) {
            ...
            if (!empty($slot['appointment_days'])) {
                ...
                $request_slots['consultation_fee'] = !empty($slot['consultation_fee']) ? $slot['consultation_fee'] : '';
            }
            $this->slots = serialize($request_slots);
        }
        if (!empty($request['location_type']) && $request['location_type'] == 'private_clinic') {
            $this->status = 'approved';
            $this->user_id = $doctor_id;
        } else {
            $this->status = 'pending';
        ...
        $previous_saved_days = !empty($profile->available_days) ? unserialize($profile->available_days) : array();
        if (!empty($previous_saved_days)) {
            $new_available_days = array_values(array_unique(array_merge($previous_saved_days,$slot['appointment_days'])));
        } else {
            $new_available_days = $slot['appointment_days'];
        }
        $profile->available_days = serialize($new_available_days);
        $profile->save();
        return 'success';
    }
    ...
    public function updateAppointmentSlots($id, $request)
    {
        if (!empty($id)) {
            ...
            if (!empty($team)) {
                ...
                $new_available_days = array();
                if (!empty($request['day']) && !in_array($request['day'], $previous_saved_days)) {
                    $new_available_days = $previous_saved_days;
                    $new_available_days[] = $request['day'];
                } else {
                    $new_available_days = $previous_saved_days;
                }
                $profile->available_days = serialize($new_available_days);
        ...
        if (!empty($id) && !empty($slot) && !empty($day)) {
            ...
            if (!empty($team)) {
                ...
                if (!empty($default_slots)) {
                    ...
                    if (($key = array_search($day,  $stored_slots['days'])) !== false) {
                        unset($stored_slots['days'][$key]);
                    }
    ...
    {
        if (!empty($doctor_id)) {
            $hospitals = Team::select('user_id', 'doctor_id')->where('doctor_id', $doctor_id)->where('status', 'approved')->get()->toArray();
            $list = array();
            if (!empty($hospitals)) {
                foreach ($hospitals as $key => $hospital) {
                    $doctor_hospital = User::find($hospital['user_id']);
                    $list[$key]['id'] = $doctor_hospital->id;
                    if ($hospital['user_id'] == $hospital['doctor_id']) {
                        $private_clinic_title = !empty($doctor_hospital->metaValue('default_location')['meta_value']) ? unserialize($doctor_hospital->metaValue('default_location')['meta_value'])['location_title']  : '';
                        $list[$key]['name'] = $private_clinic_title;
                    } else {
                        $list[$key]['name'] = Helper::getUserName($doctor_hospital->id);
                    }
                    $list[$key]['type'] = $hospital['user_id'] == $hospital['doctor_id'] ? trans('lang.doctor_private_clinic') : '';
                }
                return $list;
            } else {
                return '';
