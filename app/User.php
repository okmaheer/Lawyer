...
class User extends Authenticatable
{
     ...
     */
    public function storeUser($request, $verification_code = '', $verification_step = '', $registration_type = '', $verification_type = '')
    {
        if (!empty($request)) {
    ...
    public function updateUser($request, $edit_user_id = '')
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['role']) && $request['role'] != $role_type) {
                ...
                $profile = new UserMeta();
                $profile->user()->associate($edit_user_id);
                $profile->save();
                $role_type = Helper::getRoleTypeByUserID($edit_user_id);
                if ($role_type == 'doctor') {
     ...
     * @return \Illuminate\Http\Response
     */
    public static function getSearchableResult($type, $keyword, $search_locations, $search_service, $search_speciality, $order_by, $sort_by)
    {
        $json = array();
        $user_id = array();
        $filters = array();
        ...
        if (!empty($users)) {
            ...
            if (!empty($search_locations)) {
                ...
                $location = Location::select('id')->where('slug', $search_locations)
                    ->first();
                $locations = Location::select('id')->where('id', $location->id)->orWhere('parent', $location->id)
