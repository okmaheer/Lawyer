<template>
    <div>
        <div class="dc-formtheme dc-skillsform dc-pagesorder">
            <transition name="fade">
                <div v-if="isShow" class="sj-jump-messeges">{{ trans('lang.no_record') }}</div>
            </transition>
            <fieldset>
                <div class="dc-multiple-input">
                    ...
                    <div class="form-group" v-if="parent_menus && parent_menus.length > 0">
                    ...
                    <div class="form-group dc-btnarea">
                        <a href="javascript:void(0);" class="dc-btn" @click="addCustomLink">{{trans('lang.add_custom_link')}}</a>
                    </div>
                </div>
            </fieldset>
        </div>
            ...
            <ul id="skill_list" class="sortable list">
                <li v-for="(menuItem, index) in stored_menus" :key="index" v-if="stored_menus" class="skill-element" :ref="'skill-'+index">
                    ...
                    <div class="dc-rightarea">
                        <!-- <a href="javascript:void(0);" class="dc-addinfo" @click="editInput(index)"><i class="lnr lnr-pencil"></i></a> -->
                        <a href="javascript:void(0);" class="dc-deleteinfo delete-skill" @click="removeStoredPage(index, menuItem.custom_slug)"><i class="lnr lnr-trash"></i></a>
                    </div>
                </li>
                <li v-for="(customItem, index) in custom_links" :key="index+customItem.count">
                    ...
                    <div class="dc-rightarea">
                        ...
                        <a href="javascript:void(0);" class="dc-deleteinfo" @click="removeOrder(index, customItem.custom_slug)"><i class="lnr lnr-trash"></i></a>
                    </div>
                </li>
            </ul>
        </div>
    ...
    methods: {
        ...
        addCustomLink: function () {
            if (this.menu.custom_title == '') {
                ...
                var custom_list_count = jQuery('.dc-btn').parents('.dc-skillsform').next('.dc-myskills').find('ul#skill_list li').length;
        ...
        removeStoredPage: function (index, slug) {
            ...
            this.$swal({
                ...
                var self = this;
                $(".dc-storedinnerpageorder input[value='"+slug+"']").parent().parent('li').remove();
                if(result.value) {
                    var count_index = 0
                    self.parent_menus.forEach(element => {
                        if (element.custom_slug == slug) {
