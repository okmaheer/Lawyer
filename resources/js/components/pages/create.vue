      ...
      <div class="at-verticalscrollbar">
        <form method="POST" id="pages" class="wt-formtheme wt-formprojectinfo wt-formcategory" @submit.prevent="submitPage()">
          ...
          <div class="amt-section-content-area" id="amt-section-content-area" style="display:none;">
            ...
            />
            <image-form
              :image="form.meta.images[this.currentElementIndex]"
          ...
          <div class="at-account-save__button">
            ...
            <router-link :to="{ name: 'showPage', params: { slug: form.slug }}" class="at-actionbtn at-edit" v-if="form.slug">
              <i class="fa fa-eye"></i>
            </router-link>
          </div>
        </form>
...
export default {
  props:['section_list', 'layout_list', 'selected_search_form_type'],
  order: 5,
  components: {
    headerStyling,
    headers,
  ...
  methods: {
    ...
    async getParentPages () {
      var id = '' 
      var self = this
      await axios.get(APP_URL +'/pages/get-parents')
        .then(function (response) {
      ...
      axios.post(APP_URL +'/admin/store-page', self.form)
        .then(function (response) {
          if (response.data.type == 'success') {
            ...
            self.form.slug = response.data.slug
    ...
    updateSection: function (evt) {
      if (evt.added) {
        ...
        setTimeout(function () {
          ...
          } else if (evt.moved.section == 'two_column') {
