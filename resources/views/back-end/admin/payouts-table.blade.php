            ...
            <tbody>
                @foreach ($payouts as $key => $payout)
                    <tr>
                        ...
                        <td style="border-top: 1px solid #eff2f5;color: #767676;font-size: 13px;line-height: 20px;padding: 10px 20px;text-align: left;">
                            <!-- {{{ clean($payout->status) }}} -->
                            <span class="bt-content change-payout-status">
                                <div class="form-group">
                                    <span class="wt-select">
                                        <select id="{{$payout->id}}-payout_status" style="{{$payout->status == 'completed' ? 'pointer-events: none;' : '' }}">
                                            @foreach ($payout_status as $key => $status)
                                                <option value="{{$key}}" {{$key == $payout->status ? 'selected' : '' }}>{{$status}}</option> 
                                            @endforeach
                                        </select>
                                    </span> 
                                    <a href="javascrip:void(0);" class="wt-searchgbtn" @click.prevent='changePayoutStatus({{$payout->id}}, {{json_encode($payout->product_ids)}})'><i class="fa fa-check"></i></a>
                                </div>
                            </span>
                        </td>
                    </tr>
