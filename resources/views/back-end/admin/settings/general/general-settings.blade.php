{!! Form::open(['url' => '', 'class' =>'dc-formtheme dc-userform', 'id' =>'general-setting-form', '@submit.prevent'=>'submitGeneralSettings'])!!}
        ...
        <div class="dc-sidepadding">
            <div class="dc-settingscontent">
                ...
                <ul class="dc-accountinfo">
                    <li>
                        <switch_button v-model="enable_loader">
                            <span>{{{ trans('lang.enable_disable_loader') }}}</span>
                        </switch_button>
                        <input type="hidden" :value="enable_loader" name="enable_loader">
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="dc-location dc-tabsinfo">
        <div class="dc-tabscontenttitle">
            <h3>{{{ trans('lang.search_form_type') }}}</h3>
        </div>
        <div class="dc-settingscontent">
            <div class="dc-formtheme dc-userform">
                <fieldset>
                    <div class="form-group">
                        <span class="dc-select">
                            <select class="form-control" name="search_form_type">
                                @foreach ($search_form_types as $key => $search_type)
                                    @php $selected = $key == $selected_search_form_type ? 'selected' : ''; @endphp
                                    <option value="{{ $key }}" {{ $selected }}> {{ clean($search_type) }}</option>
                                @endforeach
                            </select>
                        </span>
                    </div>
