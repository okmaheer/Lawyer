        ...
        <div class="row">
            ...
            @php 
                if (Schema::hasColumn('appointments', 'type')) {
                    $pending_bal = Auth::user()->appointments()->where('status',  'accepted')->where('type', 'online')->where('payout_progress', null)->orWhere('payout_progress', '!=', 'processed')->sum('charges');
                    $available_bal = Auth::user()->appointments()->where('status', 'accepted')->where('type', 'online')->where('payout_progress', 'processed')->sum('charges');
                } else {
                    $pending_bal = Auth::user()->appointments()->where('status',  'accepted')->where('payout_progress', null)->orWhere('payout_progress', '!=', 'processed')->sum('charges');
                    $available_bal = Auth::user()->appointments()->where('status', 'accepted')->where('payout_progress', 'processed')->sum('charges');
                }
            @endphp
            <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-3">
                <div class="dc-insightsitem dc-dashboardbox">
                    ...
                    <div class="dc-insightdetails">
                        <div class="dc-title">
                            <h3>{{ !empty($symbol['symbol']) ? $symbol['symbol'] : '$' }}{{ number_format((float)$pending_bal, 2, '.', '') }}</h3>
                            <span>{{ trans('lang.pending_balance') }}</span>
                        </div>													
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-3">
                <div class="dc-insightsitem dc-dashboardbox">
                    ...
                    <div class="dc-insightdetails">
                        <div class="dc-title">
                            <h3>{{ !empty($symbol['symbol']) ? $symbol['symbol'] : '$' }}{{ number_format((float)$available_bal, 2, '.', '') }}</h3>
