...
@include('includes.pre-loader')
    ...
    <div class="dc-haslayout dc-main-section" id="user-profile">
        ...
        @if ($display_chat == true)
            @if (Auth::user())
                @if ($user->id != Auth::user()->id && Helper::getRoleTypeByUserID(Auth::user()->id) != 'hospital')
                    <chat 
            ...
            <div class="dc-appointmentpopup">
                <div class="dc-modalcontent modal-content">
                    <section id="sec1" class="sec1">
                        {!! Form::open(['class' => 'dc-formtheme', 'id' => 'submit_appointment_form', '@submit.prevent'=>'checkAppointmentStep1("'.$user->id.'")']) !!}
